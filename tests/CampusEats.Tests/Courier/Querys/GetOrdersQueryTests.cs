...
using CampusEats.Infrastructure.Services.CourierServices;
using CampusEats.Web;
using MediatR;
...
public class GetOrdersQueryTests
{
    ...
    [Test]
    public async Task GetOrdersQuery_ReturnsCourierOrders()
    {
    ...
    [Test]
    public async Task GetOrdersQuery_ReturnsEmptyListWhenNoOrders()
    {
        var query = new GetCourierOrdersQuery.GetCourierOrdersQueryRequest { CourierId = Guid.NewGuid() };
        var orders = await _mediator.Send(query);
    ...
    public async Task GetOrdersQuery_ReturnsCorrectOrderDetails()
    {
        ...
        Assert.That(orders[0].Status, Is.EqualTo(OrderStatus.Accepted));
    }

    [Test]
    public async Task GetOrdersQuery_ReturnsOnlyCourierOrders()
    {
        var courierId = Guid.NewGuid();
        ...
        var order2 = await CreateOrder();
        order2.CourierId = otherCourierId;
    ...
    [Test]
    public async Task GetOrdersQuery_ReturnsOrdersInCorrectStatus()
    {
        var courierId = Guid.NewGuid();
        ...
        Assert.That(orders, Has.Count.EqualTo(2));
        Assert.That(orders[0].Status, Is.EqualTo(OrderStatus.Accepted));
        Assert.That(orders[1].Status, Is.EqualTo(OrderStatus.Delivered));
    }
